...
const replaceMdSyntax = (e) => e.replace(/\[(.*?)\]\(.*?\)/g, "$1").replace(/(\*+)(\s*\b)([^\*]*)(\b\s*)(\*+)/gm, "$3"), getFileList = async (e) => {
  ...
  for (const s of n)
    s.isDirectory() ? i = [...i, ...await getFileList(`${e}/${s.name}`)] : s.name.endsWith(".md") && i.push(`${e}/${s.name}`);
  return i;
...
`), path: i };
  ...
  let n = e.anchor.replace(" ", " ").replace("\r", "").toLowerCase();
  n[0] == "#" && (n = n.replace("#", ""));
  let s = e.path.replace(rootPath + "/", "").replace("md", "html");
  return i.includes(".0") || (s += `#${n}`), {
    id: i,
    link: s,
    b: e.content,
    a: n
        ...
        if (j = f[g], x(j) || (R = j, j = R.field, e = R.query || e, i = R.limit || i), k)
          ...
          if (s ? y = s[g] : y = this.index[j].search(e, i, R || n), B = y && y.length, p && B) {
            ...
            for (let z = 0, $, H; z < p.length; z++)
              $ = p[z], (B = (H = this.l[$]) && H.length) && (q++, _[_.length] = m ? [H] : H);
